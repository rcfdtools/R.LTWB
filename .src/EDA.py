...
for parameter in parameter_list:
    ...
    for station in station_list:
        ...
        plt.title('Time serie for %s - Station %s (%d records)' % (parameter, station, df.shape[0]))
        fig_name = 'Plot_' + parameter + '_' + station + '_TimeSerie.png'
        plt.savefig(path + 'Graph/' + fig_name)
        print_log('\n![R.LTWB](Graph/%s)' %fig_name, center_div=True)
        plt.close('all')
        ...
        plt.title('Histogram for %s - Station %s (%d records)' % (parameter, station, df.shape[0]))
        ...
        plt.title('KDE density for %s - Station %s (%d rec.)' % (parameter, station, df.shape[0]))
    ...
    fig.set_ylabel(value_name)
    plt.title('Time series for %s with %d stations (%d rec.)' % (parameter, len(station_list), ideam_regs_query))
    ...
    plt.title('KDE density for %s with %d stations (%d rec.)' % (parameter, len(station_list), ideam_regs_query))
