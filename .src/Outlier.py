...
print_log('\nOutliers stats')
pd_count = pd.DataFrame(outliers.count(), columns=['Count'])
pd_max = pd.DataFrame(outliers.max(), columns=['Max'])
pd_min = pd.DataFrame(outliers.min(), columns=['Min'])
pd_concat= pd.concat([pd_count, pd_max, pd_min], axis='columns')
