...
print_log('\nOutliers stats')
pd_count = pd.DataFrame(outliers.count(), columns=['OutCount'])
pd_min = pd.DataFrame(outliers.min(), columns=['OutMin'])
pd_max = pd.DataFrame(outliers.max(), columns=['OutMax'])
pd_concat= pd.concat([pd_count, pd_min, pd_max ], axis='columns')
print_log(pd_concat.to_markdown())
