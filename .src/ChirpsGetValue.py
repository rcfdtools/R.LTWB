...
print_log('\n\n#### Correlation values for the date records\n\nThe following table, shows the monthly average correlation values obtained from the IDEAM records and the correspondent Chirps values. Get the table [%s](%s) \n' %(station_file_corr_date, station_file_corr_date))
...
print_log('![R.LTWB](PlotDateCorrelationTimeSerie.png)')
print_log('\n\n#### Average correlations per method\n\nThe values shown below, correspond to the average correlation values in each date processed. Get the table [%s](%s) \n' %(station_file_corr_date_mean, station_file_corr_date_mean))
...
print_log('![R.LTWB](PlotMonthCorrelationTimeSerie.png)')
plt.show()
